...
// _scrollController fetches the next paginated data when the current position of the user on the screen has surpassed
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        // Retrieve the 'from' parameter and address provider
        ...
        if (from == "checkout") {
          ...
          } else if (addressProvider.addresses.length == 1) {
            Navigator.pop(context, addressProvider.addresses[0]);
          } else {
            Navigator.pop(context, "");
          }
        } else if (from == "quick_widget") {
          Navigator.pop(context);
        } else {
          Navigator.pop(context, "");
        }

        // Return false to indicate that the default back navigation behavior should not be executed
        return false;
      },
      child: Scaffold(
        appBar: getAppBar(
