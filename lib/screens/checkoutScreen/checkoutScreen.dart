...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        // Check if the back navigation was invoked
        if (context.read<CheckoutProvider>().isPaymentUnderProcessing) {
          // Show a warning message if payment is under processing
          showMessage(
            context,
            getTranslatedValue(context, "you_can_not_go_back_until_payment_cancel_or_success"),
            MessageType.warning,
          );
          // Return false to prevent the default back navigation
          return false;
        } else {
          // Perform the pop action
